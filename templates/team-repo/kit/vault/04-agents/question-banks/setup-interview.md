...
(5) in the section that covers the riskiest area of the domain, after its core questions: a "### Pushback round" block with a guidance comment naming the skeptic, then PB-1 to PB-5, each as "### PB-<n> · core" testing one of value, risk, proof, difference, and fit (adapted to the domain), with Ask, a Fills line naming that file's question-and-answer section (when the domain file has none: its section ending in "to avoid", or company.md → What makes us different) and its section whose heading ends in "to avoid", Complete when, If vague, follow up, and "Skip if: never."
